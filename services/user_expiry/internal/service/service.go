...
func (c *CornService) UserExpiryCron(ctx context.Context) {
    ...
    // Every 5 minutes — hard deactivate (disconnect + lock) users that are already marked deactivated/locked
    _, err4 := cronJob.AddFunc("0 */5 * * * *", func() {
        c.HardDeactivateUsers(ctx, db)
    })
    if err4 != nil {
        logger.Fatal("Failed to add cron job: %v", err4)
    }
...
// HardDeactivateUsers is the ONLY new behavior that was requested.
// It runs every 5 minutes and simply disconnects + locks any user
// that has deactivated_at set or is_locked = true.
// No session-ID logic, no ShowSessionsValid, no extra webhook routes.
func (c *CornService) HardDeactivateUsers(ctx context.Context, db *gorm.DB) {
    var users []commonModels.OcservUser

    err := db.WithContext(ctx).
        Where("deactivated_at IS NOT NULL OR is_locked = ?", true).
        Find(&users).Error
    if err != nil {
        logger.Error("Failed to get users for hard deactivation: %v", err)
        return
    }
    if len(users) == 0 {
        return
    ...
    for _, u := range users {
        ...
        go func(u commonModels.OcservUser) {
            ...
            // Force-kill active session(s) for this user
            if c.dockerMode {
                if _, err := c.occtlDockerRepo.DisconnectUser(u.Username); err != nil {
                    logger.Error("Failed to disconnect user %s: %v", u.Username, err)
                }
            } else {
                // ← FIXED: use occtlHandler (correct interface) exactly like ExpireUsers does
                if _, err := c.occtlHandler.DisconnectUser(u.Username); err != nil {
                    logger.Error("Failed to disconnect user %s: %v", u.Username, err)
                }
            }

            // Lock the user
            var lockFunc func(string) (string, error)
            if c.dockerMode {
                lockFunc = c.occtlDockerRepo.Lock
            } else {
                lockFunc = c.ocservUserHandler.Lock
            }
            if _, err := lockFunc(u.Username); err != nil {
                logger.Error("Failed to lock user %s: %v", u.Username, err)
            }

            // Ensure DB flags are set
            now := time.Now()
            if err := db.Model(&u).Updates(map[string]interface{}{
                "is_locked":      true,
                "deactivated_at": &now,
            }).Error; err != nil {
                logger.Error("Failed to update user %s: %v", u.Username, err)
            }
        }(u)
    }
