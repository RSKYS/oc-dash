...
func (ctl *Controller) CreateOcservUser(c echo.Context) error {
    ...
    if data.TrafficType == models.Free {
        data.TrafficSize = 0
    } else if data.TrafficSize < 0.01 {
        return ctl.request.BadRequest(c, errors.New("traffic_size must be at least 0.01"))
    }
...
func (ctl *Controller) UpdateOcservUser(c echo.Context) error {
    ...
    // Updated TrafficSize / TrafficType logic (uses the prospective next type for validation)
    nextTrafficType := ocservUser.TrafficType
    if data.TrafficType != nil {
        nextTrafficType = *data.TrafficType
    }

    if data.TrafficSize != nil {
        if nextTrafficType != models.Free && *data.TrafficSize < 0.01 {
            return ctl.request.BadRequest(c, errors.New("traffic_size must be at least 0.01"))
        }
    ...
    if ocservUser.TrafficType == models.Free {
        ocservUser.TrafficSize = 0
    }
...
func (ctl *Controller) SyncToDB(c echo.Context) error {
    ...
    if data.TrafficType != nil && *data.TrafficType != models.Free {
        if data.TrafficSize == nil || *data.TrafficSize < 0.01 {
            return ctl.request.BadRequest(c, errors.New("traffic_size must be at least 0.01"))
        }
    } else if data.TrafficType != nil && *data.TrafficType == models.Free && data.TrafficSize != nil {
        *data.TrafficSize = 0
    }
